...
KLoggerInit(
	...
	SIZE_T RingBufSize = GetRingBufSize(RegistryPath);		//non checked moment
	...
	gKLogger->pFlushDpc = (PKDPC)ExAllocatePool(NonPagedPool, sizeof(KDPC));	//it differs
	if (!gKLogger->pFlushDpc) {
		Err = ERROR_NOT_ENOUGH_MEMORY;
		goto err_dpc_mem;
	}

	KeInitializeDpc(gKLogger->pFlushDpc, SetWriteEvent, NULL); //it differs
